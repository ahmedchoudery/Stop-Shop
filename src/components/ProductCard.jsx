...
const ProductCard = ({ product, onSelectProduct, onImageLoad }) => {
  ...
  // Track if this specific product's main image has loaded to prevent re-firing on hover
  const [hasLoaded, setHasLoaded] = useState(false);

  useEffect(() => {
    // Reset load state if the actual product changes
    setHasLoaded(false);
  }, [product.id]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 sm:top-3 sm:right-3 z-10">
        <span className="bg-yellow-400 text-black text-[8px] sm:text-[9px] font-black px-2 py-0.5 rounded shadow-sm uppercase tracking-tighter">
          Trending
        </span>
      </div>
      ...
      <div className="aspect-square overflow-hidden bg-transparent relative">
        <img
          src={isHovered ? product.lifestyleImage : currentImage}
          alt={product.name}
          onLoad={handleLoad}
          className={`w-full h-full object-cover mix-blend-multiply transition-all duration-700 ${isHovered ? 'scale-105' : 'scale-100'} ${product.stock === 0 ? 'grayscale opacity-70' : ''}`}
        />
        ...
        {/* Action Button - Brand Red #F63049 or Grey for Sold Out */}
      ...
      <div className="p-4 flex flex-col flex-grow text-left">
        {/* Color Swatches - Positioned just above the name */}
