 ...
 * Collects customer details, payment method, and coupon code.
 * Calls onComplete({ firstName, lastName, email, address, city, zip,
 *                    paymentMethod, finalTotal, couponCode })
 *
 * Props:
 *   onComplete(formData)  — called on submit with full form data
 *   stockWarnings []      — stock error messages from parent
 *   isSubmitting bool     — disables submit while order is being placed
 */

import React, { useState, useCallback } from 'react';
import {
  User, Mail, MapPin, CreditCard, ChevronDown,
  AlertTriangle, Lock, CheckCircle, Loader, Tag
} from 'lucide-react';
      ...
      {cartItems.map((item, i) => (
        <div key={`${item.cartId ?? item.id}-${i}`} className="flex items-start space-x-3">
          ...
          <div className="flex-grow min-w-0">
            <p className="text-[10px] font-black uppercase tracking-tight text-gray-900 truncate">
              {item.name}
            </p>
            <div className="flex flex-wrap gap-1 mt-0.5">
              {item.selectedSize && (
                <span className="text-[8px] font-bold text-gray-400 uppercase">Size: {item.selectedSize}</span>
              )}
              {item.selectedColor && (
                <span className="text-[8px] font-bold text-gray-400 uppercase">· {item.selectedColor.split('|').pop()}</span>
              )}
            </div>
...
const CheckoutForm = ({ onComplete, stockWarnings = [], isSubmitting = false }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onComplete({
      ...form,
      // ── Coupon fields ──────────────────────────
      finalTotal:  coupon?.finalTotal ?? total,   // discounted total if coupon applied
      couponCode:  coupon?.code       ?? '',       // code for server to increment usedCount
    });
  };
          ...
          <section>
            ...
            <div className="space-y-2">
              {PAYMENT_METHODS.map(({ value, label, sub }) => (
                <label
                  key={value}
                  className={`flex items-center space-x-4 p-4 border-2 cursor-pointer transition-all duration-200 ${
                ...
                >
                  ...
                  <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${
                    ...
                    {form.paymentMethod === value && (
                      <div className="w-2 h-2 rounded-full bg-[#ba1f3d]" />
                    )}
                  </div>
              ...
              {errors.paymentMethod && (
                <p className="text-[9px] font-bold text-[#ba1f3d] mt-1 flex items-center space-x-1">
                  <AlertTriangle size={9} />
