...
/**
 * Webhook handler for Stitch payments integration.
 * Enforces signature validation and is protected by idempotency key check.
 */
async function stitchWebhookHandler(req) {
  try {
    await dbConnect();
    const body = await req.json();

    const signature = req.headers.get('x-stitch-signature') || req.headers.get('x-signature');
    if (!signature) {
...
export const POST = withIdempotency(stitchWebhookHandler);
