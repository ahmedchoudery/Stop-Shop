...
- **Accessories Size Restriction**: Products under the `Accessories` category (such as watches, chains, rings, bags, caps, etc.) must not have size variants. The size variants editor section must be hidden, and sizes must be automatically kept empty (`sizes = []`) so that they only support color variants.
- **Accessories Fallback & Sizing Tab Hide**: For `Accessories` products:
  - Default description fallback must be: `Premium, custom-crafted accessory engineered with the finest detailing, offering an exceptional luxury structure.` (replacing `apparel`).
  - Default care instructions fallback must be: `Handle with care. Avoid direct contact with water, perfume, and harsh chemicals. Store in a dry place.` (replacing dry clean/washing instructions).
  - The Sizing tab must be hidden entirely on the client product detail page.
