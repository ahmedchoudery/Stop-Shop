...
- **Accessories Size Restriction**: Products under the `Accessories` category (such as watches, chains, rings, bracelets, cuffs, wallets, bags, caps, shawls, mufflers) must not have size variants. The size variants editor section must be hidden, and sizes must be automatically kept empty (`sizes = []`) so that they only support color variants.
- **Strict No-Auto-Generated Editorial Tabs (Description, Materials, Care)**: The product detail page features 3 dynamic tabs: `Description`, `Materials`, and `Care`. The system MUST NEVER automatically generate, output, or fall back to fake or hardcoded text for any of these fields. The admin inputs and edits `Description`, `Materials`, and `Care` directly through the Admin Dashboard product form. If the admin leaves any of these 3 fields empty, that specific tab is hidden from the product page. If all 3 are left empty, the editorial tabs section is hidden completely.
- **Nodemailer SMTP Fallback**: If the Resend API fails (e.g., due to sandbox recipient restrictions or verification issues), `sendEmail` must transparently fall back to Nodemailer SMTP using the configured Gmail SMTP credentials (`email_user` and `email_pass`), ensuring reliable delivery of all restock and transactional emails.
