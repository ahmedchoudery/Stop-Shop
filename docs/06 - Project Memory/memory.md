...
- **E2E Admin Endpoint & 2FA Schema Alignment**: Corrected `adminLogin` fetch endpoint in `src/lib/auth.js` from `/api/admin/login` to `/api/v1/auth/login`, added missing `/admin/login` route in `App.jsx`, aligned 2FA property check in `LoginPage.tsx` (`requiresTwoFactor`), allowed `cartTotal` string coercion in `couponValidationSchema`, added `minWidth={0} minHeight={180}` to Recharts `ResponsiveContainer` in `InventoryHealthChart.jsx` and `RevenueChart.jsx`, and configured Playwright `webServer` to use direct IPv4 loopback (`127.0.0.1:3000`).
